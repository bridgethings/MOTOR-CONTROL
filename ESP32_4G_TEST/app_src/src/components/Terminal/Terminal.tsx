...
const Terminal: FC<TerminalProps> = () => {
  ...
  const appendLog = (logText: string) => {
    ...
    lines.forEach((line, index) => {
      ...
      let color = 'slate.4';
      if (line.includes('[ERROR]') || line.includes('fail') || line.includes('Failed')) color = 'red.5';
      else if (line.includes('[MODBUS]')) color = 'cyan.4';
      else if (line.includes('[TELEM]') || line.includes('[MQTT]')) color = 'emerald.4';
      else if (line.includes('[BLE]')) color = 'indigo.4';
      else if (line.includes('OK') || line.includes('success')) color = 'emerald.5';

      setContent((prev) => [
        ...prev,
        <Group key={`log-${Date.now()}-${index}`} gap={4} wrap="nowrap" align="flex-start">
          <Text size="xs" c="slate.6" ff="monospace" style={{ flexShrink: 0 }}>
        ...
        <Stack align="stretch" justify="space-between" flex={1}>
          <Box
            style={(theme) => ({
              ...
              border: `1px solid ${theme.colors.slate[8]}`,
          ...
          >
            {/* Terminal Header */}
            <Group justify="space-between" px="md" py="xs" style={(theme) => ({ background: theme.colors.slate[8], borderBottom: `1px solid ${theme.colors.slate[7]}` })}>
              <Group gap={6}>
                ...
                <Text size="xs" c="slate.4" ml="xs" fw={600} ff="monospace">SYSTEM CONSOLE</Text>
              </Group>
              <Badge size="xs" color={isStreaming ? 'emerald' : 'slate'} variant="dot">
            ...
            >
              <Stack gap={2}>
                {content.length === 0 && (
                  <Text c="slate.6" size="xs" ff="monospace">Ready for connection...</Text>
                )}
                {content}
              </Stack>
            </ScrollArea>
            ...
            >
              <Tooltip label={isStreaming ? 'Stop Live Logs' : 'Start Live Logs'}>
                <ActionIcon
                  variant="filled"
                  radius="md"
                  color={isStreaming ? 'emerald.6' : 'slate.7'}
            ...
            <Switch
              ...
              labelPosition="left"
              color="emerald"
              styles={{ label: { color: '#94a3b8', fontSize: '10px', fontWeight: 700 } }}
              checked={echoing}
