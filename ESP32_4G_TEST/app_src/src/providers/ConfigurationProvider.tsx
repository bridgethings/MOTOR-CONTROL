...
const ConfigurationProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const reboot = useCallback(async () => {
    ...
    try {
      await api.reboot();
      notifications.show({ message: 'Device is rebooting...', color: 'blue' });
    } catch (error) {
      handleError(error, 'Reboot device');
    }
  }, [api]);
  ...
  const factoryReset = useCallback(async () => {
    ...
    try {
      ...
      notifications.show({ message: 'Factory reset complete. Device is rebooting...', color: 'blue' });
    } catch (error) {
