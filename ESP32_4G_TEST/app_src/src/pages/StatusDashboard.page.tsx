...
const StatusDashboardPage: FC = () => {
  ...
  const handleGetStatus = async () => {
    ...
    try {
      ...
      notifications.show({
        message: `Get status failed: ${err instanceof Error ? err.message : 'Unknown error'}`,
        color: 'red',
      });
    } finally {
  ...
  const modbusSlaves = modbusLiveData?.results || status?.modbus?.slaves || [];
    ...
    <Stack gap="md">
      ...
      <ConfigCard title="Modbus Register Readings" icon={<IconRefresh size={20} />}>
        ...
        {modbusSlaves.length === 0 ? (
          <Text c="dimmed" ta="center" py="xl" italic>
            ...
            {modbusSlaves.map((slave: any, idx: number) => {
              ...
              return (
                <Accordion.Item key={slaveId} value={String(slaveId)}>
                  ...
                  <Accordion.Panel>
                    {registers.length === 0 ? (
                      <Text size="sm" c="dimmed" ta="center" py="md" italic>
